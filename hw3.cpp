...
int main() {
    ...
    cout << "Number of workers is: " << worker_num << endl;
        ...
        {// Code inside this region runs in parallel.
            ...
            for (int i = 0; i < NUM_IT; i++) {
                if (pairs_tmp.empty()) break;
                    ...
                    pair<double, double> cur_pair = pairs_tmp.back();
                    pairs_tmp.pop_back();
                    // update @MAX
                    double tmp_max = max(g(cur_pair.first), g(cur_pair.second));
                    #pragma omp critical(dataupdate) 
                    {
                        MAX = MAX > tmp_max ? MAX : tmp_max;
                    }
                    // insert into deque @pairs_tmp
                    if ((cur_pair.second - cur_pair.first) / 2 < min_interval) { continue;}
                    double mid_point = (cur_pair.first + cur_pair.second) / 2;
                    double possible_max_left = (g(cur_pair.first) + g(mid_point) + s * (mid_point - cur_pair.first)) / 2;
                    
                    if (possible_max_left >= MAX + epsilon) pairs_tmp.push_back({cur_pair.first, mid_point});
                    double possible_max_right = (g(mid_point) + g(cur_pair.second) + s * (cur_pair.second - mid_point)) / 2;
                    if (possible_max_right >= MAX + epsilon) pairs_tmp.push_back({mid_point, cur_pair.second});
